...
function renderContact(contact) {
  const { id, name, email, picture, address: { street, suite, city, zipcode }, phone, website, company } = contact;
  return `
...
function render(contacts) {
  ...
  for (let contact of contacts) {
    contactsContainer.innerHTML += renderContact(contact);
  }
}
...
function filterHandler() {
  const filter = document.querySelector("#filterOptions");
...
function loadCities(contacts) {
  const filter = document.querySelector("#filterOptions");
  filter.innerHTML = `<option value="0">-- Select a city --</option>`;
  if (contacts.length != 0) {
    ...
    for (let contact of contacts) {
      ...
      if (!cities.includes(city)) {
        cities.push(city);
        filter.innerHTML += `<option value = "${city}">${city}</option>`;
      };
    };
  };
}
